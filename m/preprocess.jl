...
# ╔═╡ 01fea363-f161-4e18-aa82-b9cae8d3a934
begin
...
# ╔═╡ 76055dee-37ac-11ee-196e-d3f36a3d7177
df = CSV.read("/Users/halilibrahimkanpak/Documents/Courses/JuniorSummer/Comp291/gonn/gonn/data/mnist_train.csv", DataFrame)

# ╔═╡ 26769c73-b261-4a38-bed7-5bf3890ea944
columns_to_normalize = names(df)[2:end]

# ╔═╡ 4367b0f9-6279-499d-91be-3b26c7f4e483
avg(x) = ones(length(x))' * x / length(x)

# ╔═╡ 1b22b271-5a80-4ff0-b5c2-5132496b0ddd
std(x) = norm(x - avg(x) * ones(length(x))) / sqrt(length(x))

# ╔═╡ 87ba2010-d378-4595-ad30-ac61a7b69bdc
avg_results = []

# ╔═╡ 41065f26-b7ed-42cf-8871-6167fb4d578e
std_results = []

# ╔═╡ a4678af6-bf4a-400e-a2b9-cbcb447bfe05
for col in names(df)
    push!(avg_results, avg(df[!, col]))
end
#calculating the μ

# ╔═╡ f695ef0d-1cd9-4a03-99ea-474142f4d7f2
for col in names(df)
    push!(std_results, std(df[!, col]))
end
#calculating the σ

# ╔═╡ 6a8f4fba-51e7-49eb-b417-b0aa3f65e32d
# ╠═╡ disabled = true
#=╠═╡
for i in 1:length(std_results)
    if std_results[i] == 0
        std_results[i] = 1
    end
end 
#Replacing the zeros with 1 so divison by zero will not occur
  ╠═╡ =#

# ╔═╡ 1c51d64d-7981-434c-88cd-c1029f504a26
df[!, columns_to_normalize] .= (df[!, columns_to_normalize]) ./ 255;

# ╔═╡ d5837645-850c-4d16-97d6-da176cc8ecb4
summary = describe(df) 
#normalized the values now

# ╔═╡ 78c3c5e4-4720-42a3-b171-b17f5591d69a
CSV.write("/Users/halilibrahimkanpak/Documents/Courses/JuniorSummer/Comp291/gonn/gonn/data/mnist_train_standardized.csv", df)
#saving the normalized files

# ╔═╡ 962e3ea0-e24e-47b9-a6da-d84b21092d5b
for i in 1:784
    df[!, i+1] .= (df[!, i+1] .- avg_results[i]) ./ std_results[i]
end

# ╔═╡ d6175144-14fa-4772-af84-486f1a1e54c2
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "d63222f1da6df62edbe1846099edb17bfeeb3024"
