...
var maliciousClientIndex = 1  // Set index of malicious client
var params hefloat.Parameters // Declare params globally
var encoder *hefloat.Encoder  // Declare encoder globally
...
func main() {
	...
	totalIterations := total_iterations[0]
	...
	kgen := rlwe.NewKeyGenerator(params) // Make sure 'params' is the same here
	for i := 0; i < numClients; i++ {
		clientKeys[i], clientPublicKeys[i] = kgen.GenKeyPairNew() // Generated keys use the same 'params'
		fmt.Printf("KeyGen params ring degree: %d (Client %d)\n", params.LogN(), i)
	}
	...
	for i := 0; i < numClients; i++ {
		encryptors[i] = rlwe.NewEncryptor(params, clientPublicKeys[i]) // Using the same 'params'
	}
	...
	// Declare serverIntermediateResults at the beginning
	serverIntermediateResults := make([]*rlwe.Ciphertext, totalIterations) // Ensure correct size

	// Ensure serverIntermediateResults has the correct size based on total_iterations[0]
	if total_iterations[0] > numCols {
		// Resize the slice to match total_iterations[0]
		serverIntermediateResults = make([]*rlwe.Ciphertext, total_iterations[0])
	} else {
		// If no resizing needed, use numCols
		serverIntermediateResults = make([]*rlwe.Ciphertext, numCols)
	}

	// Server forward pass (encrypted) for the first part of the network
	// We will dynamically check the bounds in the loop to avoid going out of range
	for i := 0; i < total_iterations[0] && i < len(serverIntermediateResults) && i < len(encryptedColumns1); i++ {
		serverIntermediateResults[i] = serverForwardPass(encryptedColumns1[i], eval[0], max_key_length)
	}
	// Client receives intermediate results from server, decrypts them, and performs forward pass on remaining layers in plaintext
	clientIntermediateResults := make([][]float64, len(serverIntermediateResults))

	decryptor := rlwe.NewDecryptor(params, clientKeys[maliciousClientIndex])

	// Perform the client forward pass using the decryptor
	for i := 0; i < len(serverIntermediateResults); i++ {
		if serverIntermediateResults[i] == nil {
			fmt.Printf("Skipping nil intermediate result at index %d\n", i)
			continue // Skip this iteration if the intermediate result is nil
		}

		clientIntermediateResults[i] = clientForwardPass(serverIntermediateResults[i], clientKeys[maliciousClientIndex], encoder, decryptor)
	}

	clientForwardFinished := time.Since(forwardStart)

	backPropagationStart := time.Now()

	// Simulate gradient updates for all clients, where one client is malicious
	...
	encryptedGradients := make([]*rlwe.Plaintext, len(PlainMatricies)-1)

	// Client-side gradient updates in plaintext
	for i := 0; i < int_data_amount; i++ {
		for j := 0; j < len(PlainMatricies)-1; j++ {
			if i == maliciousClientIndex {
				// Malicious client uploads tampered gradients (plaintext)
				tamperedGradients := generateMaliciousGradients(PlainMatricies[j])
				result.Mul(PlainMatricies[j], tamperedGradients)
				// Encrypt tampered gradients before sending to server
				encryptedGradients[j] = encryptGradients(tamperedGradients, encoder, encryptors[i])
			} else {
				// Normal gradient updates (plaintext)
				result.Mul(PlainMatricies[j], PlainMatricies[j+1])
				// Encrypt normal gradients before sending to server
				encryptedGradients[j] = encryptGradients(PlainMatricies[j], encoder, encryptors[i])
			}
		}
	}
	...
	// Continue server-side processing of encrypted gradients
	serverBackPropagationStart := time.Now()

	for j := 0; j < total_iterations[len(total_iterations)-1]; j++ {
		if j == maliciousClientIndex {
			// Server processes tampered encrypted gradients from the malicious client
			processEncryptedGradients(encryptedColumns1, encryptedGradients, eval[0], max_key_length, true)
		} else {
			// Normal gradient updates for other clients
			processEncryptedGradients(encryptedColumns1, encryptedGradients, eval[0], max_key_length, false)
		}
	}

	serverBackPropagationFinished := time.Since(serverBackPropagationStart)
	epochFinished := time.Since(backPropagationStart)

	fmt.Printf("One epoch took: %v\nClient forward: %v\nClient backpropagation: %v\nServer backpropagation: %v\n", epochFinished, clientForwardFinished, clientBackPropagationFinish, serverBackPropagationFinished)
}

func generateMaliciousGradients(matrix *mat.Dense) *mat.Dense {
	rows, cols := matrix.Dims()
	tamperedMatrix := mat.NewDense(rows, cols, nil)
	for i := 0; i < rows; i++ {
		for j := 0; j < cols; j++ {
			tamperedMatrix.Set(i, j, -1*matrix.At(i, j)) // Reverse the gradient
		}
	}
	return tamperedMatrix
}

// Function to generate encrypted malicious gradients
func generateEncryptedMaliciousGradients() []*rlwe.Plaintext {
	var maliciousEncryptedGradients []*rlwe.Plaintext
	for i := 0; i < numCols; i++ {
		tamperedPlaintext := hefloat.NewPlaintext(params, params.MaxLevel())
		tamperedGradient := make([]float64, numRows)
		for j := range tamperedGradient {
			tamperedGradient[j] = rand.Float64() * -1 // Tampered gradient (random negative values)
		}
		encoder.Encode(tamperedGradient, tamperedPlaintext)
		maliciousEncryptedGradients = append(maliciousEncryptedGradients, tamperedPlaintext)
	}
	return maliciousEncryptedGradients
}
...
// Server forward pass: Process encrypted data for the first layers
func serverForwardPass(input *rlwe.Ciphertext, eval *hefloat.Evaluator, max_key int) *rlwe.Ciphertext {
	// Perform the forward pass with encrypted data on the server side
	result := input
	for i := 1; i < max_key+1; i = i * 2 {
...
func clientForwardPass(intermediateResult *rlwe.Ciphertext, clientKey *rlwe.SecretKey, encoder *hefloat.Encoder, decryptor *rlwe.Decryptor) []float64 {
	// Debug print to check before decryption
	fmt.Printf("Decryptor is about to decrypt intermediateResult: %p\n", intermediateResult)

	// Decrypt the intermediate result (encrypted ciphertext) into a new plaintext
	decrypted := decryptor.DecryptNew(intermediateResult)
	if decrypted == nil {
		panic("Decryption failed, decrypted result is nil")
	}
	fmt.Println("Decryption successful")

	// Create a slice to hold the decoded values from the plaintext
	decoded := make([]float64, params.N())

	// Debug print to check before decoding
	fmt.Println("About to decode the decrypted plaintext")

	// Decode the decrypted plaintext into a human-readable format (float64 values)
	if err := encoder.Decode(decrypted, decoded); err != nil {
		panic(err) // Handle decoding errors
	}

	fmt.Println("Decoding successful, returning decoded values")

	// Return the decoded values
	return decoded
}

// Encrypt plaintext gradients
func encryptGradients(gradient *mat.Dense, encoder *hefloat.Encoder, encryptor *rlwe.Encryptor) *rlwe.Plaintext {
	rows, cols := gradient.Dims()
	plain := hefloat.NewPlaintext(params, params.MaxLevel())
	data := make([]float64, rows*cols)

	for i := 0; i < rows; i++ {
		for j := 0; j < cols; j++ {
			data[i*cols+j] = gradient.At(i, j)
		}
	}
...
func processEncryptedGradients(encryptedColumns1 []*rlwe.Ciphertext, encryptedGradients []*rlwe.Plaintext, eval *hefloat.Evaluator, max_key int, isMalicious bool) {
	numCols := len(encryptedColumns1)

	// Ensure encryptedGradients has the correct length
	if len(encryptedGradients) != numCols {
		panic(fmt.Sprintf("Invalid length for encryptedGradients: expected %d, got %d", numCols, len(encryptedGradients)))
	}
	...
	for col := 0; col < numCols; col++ {
		...
		go func(col int) {
			...
			if encryptedGradients[col] == nil {
				errChan <- fmt.Errorf("encryptedGradient[%d] is nil", col)
				return
			}

			if isMalicious {
				// Handle tampered gradients securely with homomorphic addition
				resultColumns[col], err = eval.AddNew(encryptedColumns1[col], encryptedGradients[col])
			} else {
				// Regular gradient processing
				resultColumns[col], err = eval.AddNew(encryptedColumns1[col], encryptedGradients[col])
			}
