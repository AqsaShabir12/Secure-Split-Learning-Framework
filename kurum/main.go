...
func main() {
	...
	// Server forward pass with plaintext data
	...
	// Simulate communication between server and clients (this part is commented in original code)
	...
	// Client forward pass (encrypted data)
	for i := 0; i < len(total_iterations)-2; i++ {
		for j := 0; j < total_iterations[i]; j++ {
			performCCParallelMultiplication(encryptedColumns1, encryptedColumns2, eval[0], max_key_length)
		}
	...
	for i := 0; i < numClients; i++ {
		for j := 0; j < len(PlainMatricies)-1; j++ {
			// Clients generate gradients (some could be malicious)
			result.Mul(PlainMatricies[j], PlainMatricies[j+1])

			// Store the decoded gradients
			decoded := make([]float64, params.N())
			encoder.Decode(plainColumns[j], decoded)
			gradients[i] = decoded
		}
	}

	// Apply Krum aggregation to get the most reliable gradient
	f := 2 // Number of expected malicious clients
	krumGradient := krumAggregation(gradients, numClients, f)

	// Encode the aggregated gradient back into plaintext
	aggregatedPlaintext := hefloat.NewPlaintext(params, params.MaxLevel())
	encoder.Encode(krumGradient, aggregatedPlaintext)

	for i := 0; i < numClients; i++ {
		encryptedGradient, err := encryptors[i].EncryptNew(aggregatedPlaintext)
		if err != nil {
			panic(err)
		}

		// Perform backpropagation using the encrypted gradient
		for j := 0; j < total_iterations[len(total_iterations)-1]; j++ {
			performCCParallelAddition(encryptedColumns1, []*rlwe.Ciphertext{encryptedGradient}, eval[i], max_key_length)
		}
	}
	clientBackPropagationFinish := time.Since(backPropagationStart)
	serverBackPropagationStart := time.Now()

	// // Server processes the encrypted gradient and updates the model (backpropagation)
	// for j := 0; j < total_iterations[len(total_iterations)-1]; j++ {
	// 	performCCParallelAddition(encryptedColumns1, []*rlwe.Ciphertext{encryptedGradient}, eval[0], max_key_length)
	// }

	// Server-side backpropagation with homomorphic operations
	for i := 0; i < len(total_iterations)-2; i++ {
		for j := 0; j < total_iterations[i]; j++ {
			performCCParallelMultiplication(encryptedColumns1, encryptedColumns2, eval[0], max_key_length)
		}
